...
server.tool("send_sms", "Send an SMS message via Unifonic.", sendSmsSchema.shape,
...
server.tool("send_bulk_sms", "Send SMS to multiple recipients.", sendBulkSmsSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleSendBulkSms(params) }] }));

server.tool("get_message_status", "Get delivery status of a message.", getMessageStatusSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleGetMessageStatus(params) }] }));

server.tool("make_voice_call", "Make a text-to-speech voice call.", makeVoiceCallSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleMakeVoiceCall(params) }] }));

server.tool("send_whatsapp", "Send a WhatsApp message.", sendWhatsappSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleSendWhatsapp(params) }] }));

server.tool("get_balance", "Get account balance.", getBalanceSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleGetBalance(params) }] }));

server.tool("list_senders", "List registered sender IDs.", listSendersSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleListSenders(params) }] }));

server.tool("get_delivery_report", "Get delivery report for a message.", getDeliveryReportSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleGetDeliveryReport(params) }] }));
